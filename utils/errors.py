        ...
        # Ignoring as we're simply passing the error along.
            ...
            for arg in args:
                if isinstance(arg, Interaction):
                    interaction = arg
                if not interaction:
                    logger.warning(('Function %s decorated with @handle_callback_errors does not '
                        'include an Interaction object; no message will be passed to the '
                        'user as a result of this error.'))
            if not args:
                logger.warning(('Function %s decorated with @handle_callback_errors is not a class '
                    'method'))
                ...
                # A bit permissive but we're expecting this to be used correctly.
