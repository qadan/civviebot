...
async def on_guild_remove(guild: Guild):
    ...
    logger.info(
        ('CivvieBot was removed from guild %d; any attached webhook URL was '
         'removed, and any attached games and players, were flagged to be '
         'removed.'),
        guild.id
    )
...
async def on_guild_channel_delete(channel: GuildChannel):
    ...
    purge_channel(channel.id)
    logger.info(
        'Channel %s was deleted; its URL and associated data were removed.',
        channel.name
    )
...
async def on_guild_channel_update(before: GuildChannel, after: GuildChannel):
    ...
    try:
        ...
        if not after.permissions_for(bot_member).view_channel:
            ...
            logger.info(
                ('CivvieBot no longer has permission to view channel %s '
                 '(guild: %s); its URL and associated data were removed.'),
                after.name,
                after.guild.name
            )
    except NotFound:
        purge_channel(after.id)
        logger.info(
            ('Failed to find CivvieBot on an update of channel %s '
             '(guild: %s); its URL and associated data were removed.'),
            after.name,
            after.guild.name
        )
...
async def on_ready():
    ...
    if civviebot.debug_guilds:
        logger.info(
            ('CivvieBot is running with set debug_guilds. Global commands '
             'will not be created.')
        )
...
async def on_application_command_error(
    ...
    if isinstance(error.__context__, NoResultFound):
        await ctx.respond(
            ("Sorry, I couldn't find what you were looking for; please try "
             "again"),
            ephemeral=True
        )
        return
    await ctx.respond(
        ('Sorry, something went wrong trying to run the command; please try '
         'again'),
        ephemeral=True
    )
    logger.error(
        'A command encountered an error (initiated by %s in %s): %s\n%s\n%s',
        get_display_name(ctx.user),
        ctx.channel_id,
        error,
        ...
        (
            'Thrown from previous error:\n' + ''.join(
                format_list(extract_tb(error.__context__.__traceback__))
            )
