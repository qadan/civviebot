...
class NewWebhookModal(ChannelAwareModal):
    ...
    async def callback(self, interaction: Interaction):
        ...
        await interaction.response.send_message(
            content=(f'Created a new Civilization 6 Webhook URL: {url}. For setup help, click the '
                f'link or use `/{base_name} quickstart.'))
    ...
    async def callback(self, interaction: Interaction):
        ...
        with db_session():
            ...
            await interaction.response.edit_message(
                ...
                view=View(ConfirmDeleteUrlButton(url.slug)))
